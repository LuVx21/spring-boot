...
import org.luvx.boot.mybatis.mapper.common.provider.BaseDeleteProvider;
import org.luvx.boot.mybatis.mapper.common.provider.BaseInsertProvider;
import org.luvx.boot.mybatis.mapper.common.provider.BaseSelectProvider;
import org.luvx.boot.mybatis.mapper.common.provider.BaseUpdateProvider;
...
public interface BaseMapper<T> {
    ...
    default int insertSelectiveList(@Param("records") Collection<T> records) {
        int sum = 0;
        for (T record : records) {
            sum += insertSelective(record);
        }
        return sum;
    }
    ...
    @SelectProvider(type = BaseSelectProvider.class, method = "selectCustomer")
    List<T> selectCustomer(@Param("record") QueryEntity record);
}