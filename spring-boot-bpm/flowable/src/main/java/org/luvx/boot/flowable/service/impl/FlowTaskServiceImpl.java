...
import org.luvx.boot.flowable.config.FlowComment;
import org.luvx.boot.flowable.dto.FlowCommentDto;
import org.luvx.boot.flowable.dto.FlowTaskDto;
import org.luvx.boot.flowable.service.BaseFlowService;
import org.luvx.boot.flowable.service.FlowTaskService;
import org.luvx.boot.flowable.util.FlowableUtils;
...
import static org.luvx.boot.flowable.config.FlowComment.DELEGATE;
import static org.luvx.boot.flowable.config.FlowComment.NORMAL;

@Slf4j
@Service
public class FlowTaskServiceImpl extends BaseFlowService implements FlowTaskService {
    /**
     * 完成任务
     *
     * @param taskVo 请求实体参数
    ...
    public void complete(FlowTaskVo taskVo) {
        ...
        if (DelegationState.PENDING.equals(task.getDelegationState())) {
            taskService.addComment(taskId, processInstanceId, DELEGATE.getCode(), DELEGATE.getRemark());
            taskService.resolveTask(taskId, values);
        } else {
            taskService.addComment(taskId, processInstanceId, NORMAL.getCode(), NORMAL.getRemark());
            taskService.setAssignee(taskId, SecurityUtils.getUserId());
    ...
    public void completeTask(String taskId, Map<String, Object> varMap) {
        ...
        taskService.complete(taskId, varMap, true);
    }
    ...
    public void taskReturn(FlowTaskVo flowTaskVo) {
        ...
        try {
            ...
            runtimeService.createChangeActivityStateBuilder()
                    .processInstanceId(task.getProcessInstanceId())
                    .moveActivityIdsToSingleActivityId(currentIds, flowTaskVo.getTargetKey())
                    .changeState();
        } catch (FlowableObjectNotFoundException e) {
            throw new RuntimeException("未找到流程实例，流程可能已发生变化");
        } catch (FlowableException e) {
     ...
     */
    @Override
    public Object findReturnTaskList(FlowTaskVo flowTaskVo) {
        // 当前任务 task
        Task task = taskService.createTaskQuery()
                .taskId(flowTaskVo.getTaskId())
                .singleResult();
     ...
     */
    @Override
    public Object myProcess(Integer pageNum, Integer pageSize) {
...
//                        StringUtils.isBlank(flowTaskVo.getComment()) ? "取消申请" : flowTaskVo.getComment());
     ...
     */
    @Override
    public Object revokeProcess(FlowTaskVo flowTaskVo) {
        Task task = taskService.createTaskQuery()
                .processInstanceId(flowTaskVo.getInstanceId())
                .singleResult();
        if (task == null) {
...
//      Map<String, VariableInstance> variables = runtimeService.getVariableInstances(currentTask.getExecutionId());
        ...
        FlowNode myFlowNode = (FlowNode) bpmnModel.getMainProcess()
                .getFlowElement(myActivityId);
     ...
     */
    @Override
    public Object todoList(Integer pageNum, Integer pageSize) {
        // Page<FlowTaskDto> page = new Page<>();
        String userId = SecurityUtils.getUserId();
...
//             flowTask.setStartDeptName(startUser.getDept().getDeptName());
     ...
     */
    @Override
    public Object finishedList(Integer pageNum, Integer pageSize) {
        String userId = SecurityUtils.getUserId();
    ...
    public Object flowRecord(String procInsId, String deployId) {
        ...
        if (isNotBlank(procInsId)) {
            ...
            for (HistoricActivityInstance histIns : list) {
                if (isNotBlank(histIns.getTaskId())) {
                    FlowTaskDto flowTask = new FlowTaskDto();
                    flowTask.setTaskId(histIns.getTaskId());
                    flowTask.setTaskName(histIns.getActivityName());
                    flowTask.setCreateTime(histIns.getStartTime());
                    flowTask.setFinishTime(histIns.getEndTime());
                    if (isNotBlank(histIns.getAssignee())) {
                    ...
                    for (HistoricIdentityLink identityLink : linksForTask) {
                        if ("candidate".equals(identityLink.getType())) {
                            if (isNotBlank(identityLink.getUserId())) {
                                // SysUser sysUser = sysUserService.selectUserById(Long.parseLong(identityLink.getUserId()));
                                // stringBuilder.append(sysUser.getNickName()).append(",");
                            }
                            if (isNotBlank(identityLink.getGroupId())) {
                                // SysRole sysRole = sysRoleService.selectRoleById(Long.parseLong(identityLink.getGroupId()));
                                // stringBuilder.append(sysRole.getRoleName()).append(",");
                            }
                        }
                    }
                    if (isNotBlank(stringBuilder)) {
                        flowTask.setCandidate(stringBuilder.substring(0, stringBuilder.length() - 1));
                    }
...
//            }
        ...
        // 第一次申请获取初始化表单
        // if (isNotBlank(deployId)) {
        //     SysForm sysForm = sysInstanceFormService.selectSysDeployFormByDeployId(deployId);
        //     if (Objects.isNull(sysForm)) {
        //         return Object.error("请先配置流程表单");
        //     }
    ...
    public InputStream diagram(String processId) {
        ...
        //获取自定义图片生成器
        ProcessDiagramGenerator diagramGenerator = configuration.getProcessDiagramGenerator();        // new CustomProcessDiagramGenerator();
     ...
     */
    @Override
    public Object getFlowViewer(String procInsId) {
        List<HistoricActivityInstance> flowViewerList = new ArrayList<>();
        // 获得活动的节点
        ...
        for (HistoricActivityInstance activityInstance : hisActIns) {
            if (!"sequenceFlow".equals(activityInstance.getActivityType())) {
                flowViewerList.add(activityInstance);
            }
        }
     ...
     */
    @Override
    public Object processVariables(String taskId) {
        // 流程变量
        HistoricTaskInstance historicTaskInstance = historyService.createHistoricTaskInstanceQuery()
                ...
                .taskId(taskId)
                .singleResult();
        return historicTaskInstance != null ? historicTaskInstance.getProcessVariables() : taskService.getVariables(taskId);
    }
     ...
     */
    // @Override
    // public Object getNextFlowNode(FlowTaskVo flowTaskVo) {
    //     Task task = taskService.createTaskQuery()
    //             .taskId(flowTaskVo.getTaskId())
    //             .singleResult();
     ...
     * 查询全部任务
     */
    public List<HistoricTaskInstance> queryHistoryTaskList(String assignee, String taskId, String processInstanceId, String processDefinitionId) {
        ...
        if (isNotBlank(processDefinitionId)) {
